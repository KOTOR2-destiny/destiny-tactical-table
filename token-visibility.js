...
button.addEventListener('click',async()=>{if(!gm()||!current||busy)return;busy=true;button.disabled=true;const next=!current.hidden;status.textContent=next?'Hiding token from PCs…':'Revealing token to PCs…';try{const {data,error}=await db.rpc('set_token_hidden',{p_token_id:current.id,p_hidden:next});if(error||!data)throw error||Error('Visibility update rejected.');const saved=Array.isArray(data)?data[0]:data;current={id:saved.id,label:saved.label,hidden:saved.hidden};button.textContent=current.hidden?'REVEAL TO PCs':'HIDE FROM PCs';button.classList.toggle('token-reveal',current.hidden===true);status.textContent=current.hidden?current.label+' is hidden from PCs.':current.label+' is visible to PCs.';}catch(e){status.textContent='Visibility update failed: '+e.message;}finally{busy=false;button.disabled=false;}});
